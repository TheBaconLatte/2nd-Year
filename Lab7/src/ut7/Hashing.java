...
//the whole thing is added together and then moduloed to create a hash table index
        ...
        //return (int)word.charAt(word.length()-1);
         ...
         int count = 0;
         for(char c : word.toCharArray()) { //converts the word to a character array
           int i = (int) c; //converts current character to an int
           hash += i * modPow(258, count, size); //creates the hash by adding the int chars to successive powers
           count++;
         }

         hash = hash % size; 
         return hash; //you've got your hash
